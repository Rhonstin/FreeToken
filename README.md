...
- **Structured Outputs**: `response_format` (`json_object` and `json_schema`) is enforced at the sampler with an [xgrammar](https://github.com/mlc-ai/xgrammar) grammar, so OpenAI SDK `.parse()` calls get schema-valid JSON -- see [docs/structured-output.md](docs/structured-output.md); install with `freetoken[structured]`.
