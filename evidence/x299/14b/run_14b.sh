#!/usr/bin/env bash
# 14b: QSA/GDN/MoE reference tests on target + full-model A/B MoE nvfp4 backend.
set -uo pipefail
...
restart(){ echo "[14b] restart prod"; sudo -n systemctl start freetoken.service; }
trap restart EXIT
...
echo "[14b] pytest kernel tests $(date -u +%H:%M:%S)"
echo "[14b] pytest rc=$?  $(tail -1 "$RAW/pytest-kernels.txt")"

probe() {  # label port quant_backend
  local label=$1 port=$2 qb=$3
  local extra=""; [ -n "$qb" ] && extra="--quant-backend $qb"
  echo "[14b] $label qb=${qb:-default} $(date -u +%H:%M:%S)"
  ...
  if [ "$ok" = 1 ]; then
    "$PY" evidence/x299/05b/context_probe.py --origin "http://127.0.0.1:$port" --prompt-dir "$FIX" \
      --contexts 4k --decode 128 --mode "$label" --jsonl "$RAW/backend-ab.jsonl" > "$RAW/probe-$label.log" 2>&1
    echo "[14b] $label rc=$?"
  else echo "$label NOT READY" >> "$RAW/backend-ab.txt"; fi
