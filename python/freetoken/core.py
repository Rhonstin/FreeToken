...
class Req:
    ...
    # hidden]) scattered at image-token positions during this request's prefill. The
    # in-process offline path sets this directly; the online path carries ``mm_items``
    # instead and the engine encodes them into the same tensor before the forward.
