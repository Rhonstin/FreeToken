...
KV_QUANT_ALIASES = {
    "auto": "none", "bf16": "none", "none": "none", "fp8": "fp8", "nvfp4": "nvfp4",
}
...
def _resolve_auto_attention_backend(
    ...
    raise RuntimeError(
        ...
        + (f" with a {kv_quant} KV cache" if kv_quant != "none" else "")
        + "."
    )
...
class Engine:
    ...
    def _resolve_hybrid_fetch(self, config: EngineConfig, cache) -> None:
        ...
        fraction = load_hybrid_fetch_fraction(
            ...
            expert_bytes=_model_expert_bytes(cache.quant_format, config.model_config),
...
def _model_expert_bytes(bench_fmt: str, model_config) -> int | None:
    """Per-expert offload-bank bytes for this model in ``bench_fmt``, or None without one."""
...
def _adjust_config(config: EngineConfig):
    ...
    if kv_quant == "nvfp4":
        # NVFP4 is verified for the plain paged (MHA) and hybrid-SWA Triton readers and
        # for QSA sparse attention. MLA/DSA have no NVFP4 read path in this tree yet
        # (backlog 5y2.18) and the remaining sparse families never had one; reject them
        # here, before any pool is priced or any weight is resident.
        if required_attn_types - {AttnType.FULL, AttnType.SWA, AttnType.QSA}:
            raise ValueError(
                "--kv-cache-dtype nvfp4 is implemented for the plain paged, hybrid-SWA "
                "and QSA sparse KV pools; this model needs a family without an NVFP4 "
                "read path (use --kv-cache-dtype bf16)."
            )
    ...
    if kv_quant != "none":
        ...
        # kernel: plain paged and hybrid-SWA, QSA sparse, and DSA/MLA. Other sparse
        # families have no scale-read path and are rejected before weights load.
