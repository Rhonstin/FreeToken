...
# routers the family builds without a quant config (qwen3_5_moe/moe.py), so a checkpoint that
# quantized them is dequantized at load
...
_MODEL_REGISTRY: dict[str, ModelSpec] = {
    ...
    "Qwen3_5MoeForConditionalGeneration": ModelSpec(
        ...
        checkpoint_roots=_QWEN_VL_ROOTS,
        # image input for Qwen3.5 is not ported into this fork (its loader is the fork's
        # own quant path): no processor and no encoders -> the family stays text-only.
...
def checkpoint_quant_config(model_path: str, hf_config: Any, spec: "ModelSpec"):
    """The checkpoint's QuantConfig under the family's naming, or None for GGUF, whose
    native-quant ops the shared parser does not model yet.

    Lives here (not in engine/config.py) because the tests and the engine both resolve it
    from the model spec it is built around.
    """
    ...
    if spec.parse_config == "parse_gguf_config":
        return None
    # NOTE: ModelOpt exports before 0.41 keep the quantization config only in
    # hf_quant_config.json, and the weight download fetches nothing but the safetensors
    # shards, so this sidecar is fetched on its own.
...
__all__ = ["ModelSpec", "get_model_spec", "get_model_class", "checkpoint_quant_config"]
