...
def verdict(cpu_runs: list[float], pcie_runs: list[float],
    ...
    A single ratio hides how repeatable it is. Bound the ratio by the worst and best pairing
    actually observed: when that interval clears ``threshold`` the call is real; when it
    straddles it, say so and fall back to ``offload`` -- the backend that always works.
    """
...
def _bench_format(fmt: str, wl: Workload, device: torch.device, threshold: float,
                  reps: int, cpu_threads: int, cpu_iters: int, pcie_iters: int,
                  isas: list[str] | None = None) -> dict:
    ...
    if cpu_g is not None and pcie_g:  # pcie_g truthy also rules out a div-by-zero
        ...
        # threshold gets a different verdict run to run, and a single number hides that.
...
def _print_kernels(kernels: dict, iw: int) -> None:
    ...
    for fmt, e in kernels.items():
        ...
        backend = e["recommended"]
        if e.get("confident") is False:
            backend = f"{backend} (unstable)"
