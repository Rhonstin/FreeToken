...
class DetokenizeManager:
    ...
    def detokenize(self, msgs: List[DetokenizeMsg]) -> List[str]:
        ...
        for msg in msgs:
            ...
            # One message at a time: a multi-token (speculative) drain delivers several
            # messages for one uid in a single batch, and each message must advance the
            # offsets before the next delta is measured -- otherwise the following
            # message's slice still covers the previous token and its text repeats in
            # the delta (live: " need" then " need answer").
