...
@pytest.mark.parametrize(
    ("rows", "kv_heads", "topk", "bit_exact"),
    [(1, 1, 64, True), (16, 2, 64, False)],
)
def test_fp8_codes_match_the_bf16_cache_bit_for_bit(rows, kv_heads, topk, bit_exact):
    torch.manual_seed(3)
    ...
    if bit_exact:
        ...
        return
    # The direct profile (NUM_SPLITS=1) compiles a SECOND kernel instance, and on
    # sm_86/triton 3.6.0 its tl.dot accumulation order differs from the bf16 instance
    # for the computed (decoded) operands: exactly one element of 4096 lands one bf16
    # ulp away, deterministically, while the split profile is bit-exact. The operands
    # themselves are proven identical (lossless on-grid codes, power-of-two scales, same
    # fp32 multiply + bf16 narrow), so this bounds the compiler artifact alone: every
    # other element must still be bit-equal.
    diff = (got.float() - want.float()).abs()
    allowed = want.float().abs() * 2.0**-7  # >= one bf16 ulp
    bad = diff > allowed
    assert int(bad.sum()) <= 1, (
        f"{int(bad.sum())} elements beyond one bf16 ulp; max diff {diff.max().item():.3e}"
    )
