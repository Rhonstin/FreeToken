...
def test_backend_pick_follows_the_served_expert_geometry(tmp_path):
    """Upstream PR #278: the nvfp4 dtype bench (3072x1536, 7.97 MB experts) says hybrid, but a
    small-expert model (2048x512, 1.78 MB) benches offload and decodes faster that way. A
    per-model bench of the exact geometry wins; a dtype verdict >2x away is not applied."""
    small = {"hidden": 2048, "inter": 512, "experts": 256, "top_k": 8}
    prof = {
        "schema_version": 5, "fingerprint": FP,
        "gpu": {"name": "FAKE GPU"},
        "dtypes": {"nvfp4": "hybrid"},
        "dtype_kernels": {"nvfp4": {
    ...
    assert load_backend_recommendation("nvfp4", path=p, current_fingerprint=FP,
                                       expert_bytes=1775616, geometry=small) == "offload"
    assert load_hybrid_fetch_fraction("nvfp4", path=p, current_fingerprint=FP,
                                      expert_bytes=1775616, geometry=small) == pytest.approx(25.4 / (25.4 + 28.2))
    # callers that pass no geometry keep the format-only join
    assert load_backend_recommendation("nvfp4", path=p, current_fingerprint=FP) == "hybrid"

    # no bench of this geometry: a verdict from 4.5x larger experts is not applied
    del prof["workloads"]
    path.write_text(json.dumps(prof))
    assert load_backend_recommendation("nvfp4", path=p, current_fingerprint=FP,
                                       expert_bytes=1775616, geometry=small) is None
    assert load_hybrid_fetch_fraction("nvfp4", path=p, current_fingerprint=FP,
                                      expert_bytes=1775616, geometry=small) is None
    # ... while comparable experts (within 2x) inherit it as before
    assert load_backend_recommendation(
        "nvfp4", path=p, current_fingerprint=FP, expert_bytes=6_000_000,
        geometry={"hidden": 3072, "inter": 1024, "experts": 128, "top_k": 8}) == "hybrid"
    assert load_hybrid_fetch_fraction("nvfp4", path=p, current_fingerprint=FP,
                                      expert_bytes=6_000_000) == pytest.approx(25.2 / (25.2 + 74.1))
